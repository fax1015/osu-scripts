...
/** Align with session cookie / Redis session lifetime (30 days). */
export const SESSION_DATA_TTL_SEC = 60 * 60 * 24 * 30;
...
export { redisCommand as runRedis };
...
export async function persistSave(sessionId, data) {
  ...
  if (redisRestConfig()) {
    const ttl = String(SESSION_DATA_TTL_SEC);
    await redisCommand([
      "SET",
...
/**
 * Remove the session payload (Redis or local file). No-op for invalid id.
 * @param {string} sessionId
 */
export async function persistDelete(sessionId) {
  if (!validSessionId(sessionId)) {
    return;
  }
  if (redisRestConfig()) {
    try {
      await redisCommand(["DEL", sessionStorageKey(sessionId)]);
    } catch (error) {
