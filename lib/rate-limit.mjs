...
async function consumeRunSlotRedis(sessionId) {
  ...
  try {
    ...
    if (mNum > MAX_PER_MINUTE) {
      ...
      return { allowed: false, error: "Too many script runs. Try again in a minute.", retryAfterSec: 60 };
    }
    ...
    if (hNum > MAX_PER_HOUR) {
      ...
      return { allowed: false, error: "Hourly run limit reached. Try again later.", retryAfterSec: 3600 };
    }
  } catch (error) {
