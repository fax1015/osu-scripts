...
export async function handleOAuthStart(request, response) {
  ...
  try {
    ...
    sendHtml(
      ...
      authResultPage(
        [
          `osu! rejected the saved OAuth app credentials before login started: ${error.message}`,
...
export async function handleOAuthCallback(request, response, url) {
  ...
  try {
    ...
    sendHtml(
      ...
      authResultPage(
        [
          `osu! rejected the saved OAuth app credentials: ${error.message}`,
...
export async function handleRun(request, response) {
  ...
  if (script === "oldest" && !tokenResult.accessToken) {
    sendJson(response, 401, {
      ok: false,
      error: "Find oldest scores requires osu! login. Log in with osu! first, then run it again.",
    });
    return;
  }
...
export async function handleRunStream(request, response) {
  ...
  if (script === "oldest" && !tokenResult.accessToken) {
    ...
    writeNdjson(response, {
      type: "error",
      message: "Find oldest scores requires osu! login. Log in with osu! first, then run it again.",
    });
    response.end();
    return;
  }
