...
import { persistDelete, persistLoad, persistSave, ROOT_DIR } from "./persist-settings.mjs";
import { clearSessionRatelimitKeys, consumeRunSlot } from "./rate-limit.mjs";
import { clearSessionCookie, getExistingSessionId, getOrCreateSessionId } from "./session.mjs";
...
/** In-memory per-session settings for the local dev server only. */
...
async function loadSettingsBySessionId(sessionId) {
  if (sessionId && !isVercelRuntime() && sessionCache.has(sessionId)) {
    return sessionCache.get(sessionId);
  }

  const raw = await persistLoad(sessionId);
  ...
  if (sessionId && !isVercelRuntime()) {
    sessionCache.set(sessionId, next);
  }
  return next;
}

async function loadSessionSettings(request, response) {
  const sessionId = getOrCreateSessionId(request, response);
  return { sessionId, settings: await loadSettingsBySessionId(sessionId) };
}

/** Used when a session cookie is required to already exist (OAuth callback). */
async function loadSessionByExistingCookie(request) {
...
async function saveSettingsInternal(sessionId, nextSettings) {
  const next = normalizeSettings(stripOauthAppSecretsForPersistIfEnv(nextSettings));
  await persistSave(sessionId, next);
  if (sessionId && !isVercelRuntime()) {
    sessionCache.set(sessionId, next);
  }
  return next;
...
function uiReturnUrl(fragmentBody = "home") {
...
export function publicSettings(settings, request) {
  ...
  return {
    ...
    // Temporary: debug whether the client sent a valid session cookie (never exposes the id)
    session: { hasSession: Boolean(request && getExistingSessionId(request)) },
  };
}
...
export async function handleSettings(request, response) {
  const { sessionId, settings } = await loadSessionSettings(request, response);
  ...
  sendJson(response, 200, publicSettings(await saveSettingsInternal(sessionId, nextSettings), request));
}
...
export async function handleOAuthStart(request, response) {
  ...
  if (!creds.clientId || !creds.clientSecret) {
    sendHtml(
      ...
      authResultPage(
        ...
        false,
        uiReturnUrl("home?oauthError=unconfigured"),
      ),
    );
    return;
  }
  ...
  try {
    ...
    sendHtml(
      ...
      authResultPage(
        ...
        false,
        uiReturnUrl("home?oauthError=invalid_client"),
      ),
    );
    return;
  }
  ...
  await saveSettingsInternal(sessionId, {
    ...settings,
    oauth: {
      ...oauth,
      pendingState: state,
...
export async function handleOAuthCallback(request, response, url) {
  ...
  const session = await loadSessionByExistingCookie(request);
  if (!session) {
    sendHtml(
      response,
      400,
      authResultPage("no session cookie. open the site and start sign-in again.", false),
    );
    return;
  }
  ...
  if (!creds.clientId || !creds.clientSecret) {
    sendHtml(
      response,
      400,
      authResultPage("sign-in is not set up on this server.", false, uiReturnUrl("home?oauthError=unconfigured")),
    );
    return;
  }
...
export async function handleOAuthLogout(request, response) {
  ...
  if (sessionId) {
    ...
    await clearSessionRatelimitKeys(sessionId);
  }
...
function authResultPage(message, ok, returnPath = uiReturnUrl()) {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
...
async function getRunAccessToken(sessionId, settings) {
  const warnings = [];
  const envToken = process.env.OSU_ACCESS_TOKEN?.trim() || "";
  const oauth = settings.oauth;
  ...
  if (oauth.refreshToken && appCreds.clientId && appCreds.clientSecret) {
    try {
      ...
      const nextSettings = await saveSettingsInternal(sessionId, {
        ...settings,
...
export async function handleRun(request, response) {
  ...
  const rl = await consumeRunSlot(sessionId);
  if (!rl.allowed) {
    sendJson(
      response,
      429,
      { ok: false, error: rl.error || "rate limit." },
      rl.retryAfterSec ? { "retry-after": String(rl.retryAfterSec) } : {},
    );
    return;
  }

  const tokenResult = await getRunAccessToken(sessionId, settings);
