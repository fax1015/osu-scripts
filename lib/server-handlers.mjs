...
export function publicSettings(settings, request) {
  ...
  const canRefresh = Boolean(oauth.refreshToken && oauth.clientId && oauth.clientSecret);
...
function normalizeGuestSettings(input = {}) {
  ...
  return {
    ...
    output: stringValue(input.output, defaults.output),
...
export async function handleSettings(request, response) {
  ...
  const incomingClientId = body.oauth?.clientId != null
    ? stringValue(body.oauth.clientId, "")
    : settings.oauth.clientId;
  const clientIdChanged = incomingClientId !== settings.oauth.clientId;
  const oauth = clientIdChanged
    ? clearOAuthToken({ ...settings.oauth, clientId: incomingClientId, clientSecret: "" })
    : {
        ...settings.oauth,
        clientId: incomingClientId,
      };

  if (typeof body.oauth?.clientSecret === "string" && body.oauth.clientSecret.trim()) {
    oauth.clientSecret = body.oauth.clientSecret.trim();
  }

  if (body.oauth?.clearClientSecret) {
    oauth.clientSecret = "";
  }
...
export async function handleOAuthStart(request, response) {
  ...
  if (!oauth.clientId || !oauth.clientSecret) {
    sendHtml(response, 400, authResultPage("osu! login needs a Client ID and Client Secret first.", false));
    return;
  }

  try {
    await validateOAuthClient(oauth);
  } catch {
    const nextOauth = clearOAuthToken(oauth);
    nextOauth.clientSecret = "";
    await saveSettings({
      ...settings,
      oauth: nextOauth,
    });
    sendHtml(
      ...
      authResultPage(
        [
          "osu! rejected the saved OAuth app credentials before login started.",
          "Open Account & setup, paste the current Client ID and Client Secret, save settings, and try again.",
        ].join(" "),
        false,
        uiReturnUrl("setup?oauthSetup&oauthError=invalid_client"),
      ),
    );
    return;
  }
...
export async function handleOAuthCallback(request, response, url) {
  ...
  try {
    ...
    sendHtml(
      ...
      authResultPage(
        [
          "osu! rejected the saved OAuth app credentials.",
          "Open Account & setup, paste the current Client ID and Client Secret from osu!, and make sure the callback URL matches exactly.",
        ].join(" "),
        false,
        uiReturnUrl("setup?oauthSetup&oauthError=invalid_client"),
      ),
...
async function validateOAuthClient(oauth) {
...
function guestOutputPath(settings) {
  if (isVercelRuntime()) {
    return "-";
  }
  return settings.output || DEFAULT_SETTINGS.guest.output;
}

function buildGuestArgs(target, settings) {
  const args = [path.join(ROOT_DIR, "find-osu-guest-difficulties.mjs"), target];
  args.push(`--output=${guestOutputPath(settings)}`);
...
function buildOldestArgs(target, settings) {
  const args = [path.join(ROOT_DIR, "find-oldest-osu-score.mjs"), target];
  args.push(`--modes=${settings.modes.join(",")}`);
